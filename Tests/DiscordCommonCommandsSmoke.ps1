...
if (-not (Test-Path -LiteralPath $pluginPath)) { throw 'Build ServerManager before running this smoke test.' }
...
$testDirectory = Join-Path $temporaryRoot ('ServerManager-DiscordCommon-' + [Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $testDirectory | Out-Null
$ownedProcess = $null
try {
    $harness = Join-Path $testDirectory 'DiscordCommonCommandsSmoke.exe'
    ...
    $compileArgs += @("/reference:$YamlDotNetPath", "/reference:$JsonPath",
        ...
        (Join-Path $ProjectRoot 'Tests\DiscordCommonCommandsSmoke.cs'))
    & dotnet @compileArgs
    if ($LASTEXITCODE -ne 0) { throw "Discord shared adapter harness compilation failed ($LASTEXITCODE)." }
    ...
    $ownedProcess = Start-Process -FilePath $harness -ArgumentList ('"' + $pluginPath + '"') -WorkingDirectory $testDirectory -WindowStyle Hidden -RedirectStandardOutput $stdout -RedirectStandardError $stderr -PassThru
    ...
    if (-not $finished) { throw 'Discord shared adapter probe timed out; only its owned helper was stopped.' }
    if ($ownedProcess.ExitCode -ne 0) { throw "Discord shared adapter smoke failed ($($ownedProcess.ExitCode))." }
}
finally {
    ...
    if (-not $resolvedTestDirectory.StartsWith($temporaryRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase) -or
        ...
        (Split-Path -Leaf $resolvedTestDirectory) -notmatch '^ServerManager-DiscordCommon-[0-9a-f]{32}$') {
        throw 'Refusing to clean an unexpected Discord shared adapter test directory.'
    }
