...
foreach ($required in @($compiler, $YamlDotNetPath, (Join-Path $frameworkPath 'mscorlib.dll'))) {
    if (-not (Test-Path -LiteralPath $required)) { throw "Item-data preset test prerequisite missing: $required" }
}
$temporaryRoot = [IO.Path]::GetFullPath([IO.Path]::GetTempPath()).TrimEnd('\', '/')
$testDirectory = Join-Path $temporaryRoot ('ServerManager-ItemDataPresets-' + [Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $testDirectory | Out-Null
$ownedProcess = $null
try {
    $harness = Join-Path $testDirectory 'ItemDataPresetsSmoke.exe'
    $compileArgs = @(
        ...
        (Join-Path $ProjectRoot 'Commands\ItemDataPresets.cs'),
        (Join-Path $ProjectRoot 'Tests\ItemDataPresetsSmoke.cs')
    )
    & dotnet @compileArgs
    if ($LASTEXITCODE -ne 0) { throw "Item-data preset harness compilation failed ($LASTEXITCODE)." }
    ...
    if (-not $finished) { throw 'Item-data preset probe timed out; only its owned helper was stopped.' }
    if ($ownedProcess.ExitCode -ne 0) { throw "Item-data preset smoke failed ($($ownedProcess.ExitCode))." }
}
finally {
    ...
    if (-not $resolvedTestDirectory.StartsWith($temporaryRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase) -or
        ...
        (Split-Path -Leaf $resolvedTestDirectory) -notmatch '^ServerManager-ItemDataPresets-[0-9a-f]{32}$') {
        throw 'Refusing to clean an unexpected item-data preset test directory.'
    }
