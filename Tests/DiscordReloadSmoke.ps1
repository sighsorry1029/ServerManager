...
foreach ($required in @($compiler, $YamlDotNetPath, (Join-Path $frameworkPath 'mscorlib.dll'))) {
    if (-not (Test-Path -LiteralPath $required)) { throw "Reload test prerequisite missing: $required" }
}
$temporaryRoot = [IO.Path]::GetFullPath([IO.Path]::GetTempPath()).TrimEnd('\', '/')
$testDirectory = Join-Path $temporaryRoot ('ServerManager-DiscordReload-' + [Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $testDirectory | Out-Null
$ownedProcess = $null
try {
    $harness = Join-Path $testDirectory 'DiscordReloadSmoke.exe'
    $compileArgs = @(
        ...
        (Join-Path $ProjectRoot 'Discord\DiscordRuntime.cs'),
        (Join-Path $ProjectRoot 'Tests\DiscordReloadSmoke.cs')
    )
    & dotnet @compileArgs
    if ($LASTEXITCODE -ne 0) { throw "Reload harness compilation failed ($LASTEXITCODE)." }
    ...
    if (-not $finished) { throw 'Reload probe timed out; only its owned helper was stopped.' }
    if ($ownedProcess.ExitCode -ne 0) { throw "Reload smoke failed ($($ownedProcess.ExitCode))." }
}
finally {
    ...
    if (-not $resolvedTestDirectory.StartsWith($temporaryRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase) -or
        ...
        (Split-Path -Leaf $resolvedTestDirectory) -notmatch '^ServerManager-DiscordReload-[0-9a-f]{32}$') {
        throw 'Refusing to clean an unexpected reload test directory.'
    }
