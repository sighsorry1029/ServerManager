...
foreach ($required in @($compiler, $YamlDotNetPath, (Join-Path $frameworkPath 'mscorlib.dll'))) {
    if (-not (Test-Path -LiteralPath $required)) { throw "Server settings test prerequisite missing: $required" }
}
$temporaryRoot = [IO.Path]::GetFullPath([IO.Path]::GetTempPath()).TrimEnd('\', '/')
$testDirectory = Join-Path $temporaryRoot ('ServerManager-ServerSettings-' + [Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $testDirectory | Out-Null
$ownedProcess = $null
try {
    $harness = Join-Path $testDirectory 'ServerSettingsSmoke.exe'
    $compileArgs = @(
    ...
    & dotnet @compileArgs
    if ($LASTEXITCODE -ne 0) { throw "Server settings harness compilation failed ($LASTEXITCODE)." }
    ...
    if (-not $finished) { throw 'Server settings probe timed out; only its owned helper was stopped.' }
    if ($ownedProcess.ExitCode -ne 0) { throw "Server settings smoke failed ($($ownedProcess.ExitCode))." }
}
finally {
    ...
    if (-not $resolvedTestDirectory.StartsWith($temporaryRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase) -or
        ...
        (Split-Path -Leaf $resolvedTestDirectory) -notmatch '^ServerManager-ServerSettings-[0-9a-f]{32}$') {
        throw 'Refusing to clean an unexpected server settings test directory.'
    }
