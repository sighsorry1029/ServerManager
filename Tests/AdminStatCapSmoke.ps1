...
function Test-Reachable($Start, $Target, [object[]]$Blocked = @()) {
...
function Assert-Dominates($Method, $Guard, $Target, [string]$Message) {
    Assert-True ($null -ne $Guard -and $null -ne $Target -and
        -not (Test-Reachable $Method.Body.Instructions[0] $Target @($Guard))) $Message
}
function Assert-PredicateControls($Predicate, $Target, [string]$Message) {
    ...
    Assert-True ($null -ne $branch -and $branch.Offset -lt $Target.Offset -and
        ((Test-Reachable $branch.Operand $Target) -xor (Test-Reachable $branch.Next $Target))) $Message
}
...
Assert-Dominates $adminCheck (Get-Call $adminCheck 'get_HostId') (Get-Call $adminCheck 'IsAdmin') `
...
Assert-True ($null -ne $pendingBranch -and
    ((Test-Reachable $pendingBranch.Operand $policySend @($pendingGuard)) -xor
     (Test-Reachable $pendingBranch.Next $policySend @($pendingGuard)))) `
